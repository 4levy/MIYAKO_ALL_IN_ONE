...
▄███▄   ▄████▄████▄    █████████ ▀▀████▀▀      ▄████▄     ▀██████▀▀ ███████████████████   ▄████▄ ▄███▄████████████████████████████████▄   ▄████▄▀█████▀▄██████████
                                     ███                                                                                                                          
                                      ▀████▀                                                                                                                      
                                            ...
                                            LICENSE -- [ FALSE ] -- BY LEVY
DO NOT REMOVE

แนะนำให้เซฟ Config อันเก่าก่อน ก่อนที่จะทำการอัพเดทอั้นไหม
...
TIME = 5
STATUS1 = 'พร้อมใช้งาน 🟢'
STATUS2 = 'Version V2.8'
STATUS3 = 'Created by 4levy'
STATUS4 = 'Helper alzqa'
STATUS5 = "you??!.."
url_status = "https://www.twitch.tv/4levy_z1"
...
# UPTIME CONFIG
channel_uptime = '0' # ID ช่องคำสั่ง
...
button_role_1 = "SCR ZONE 1"
button_role_2 = "SCR ZONE 2"
button_role_3 = "SCR ZONE 3"
button_role_4 = "SCR ZONE 4"
button_role_5 = "SCR ZONE 5"
button_role_6 = "SCR ZONE 6"
button_role_7 = "SCR ZONE 7"
button_role_replit = "REPLIT ZONE"
...
emoji_13 = "<:emoji_1:1218006305512489040>"
emoji_14 = "<a:j2:1210538794407694396>"
emoji_15 = "<a:zeen2:1218006357450559559>"
emoji_16 = "<a:j5:1210538806634090507>"
...
import validators
import httpx
from nextcord.ui import View, Modal, Select
...
async def on_ready():
 ...
 █▀█ █▀  █▄▄ ▀▄█   █▀  █▀▄ ▀▄▀ ▀▄█ █▄▄ ▀▄▄  █ 
    ...
    a_text(f"{TerminalColors.OKBLUE}> Connected to {len(bot.guilds)} servers :{TerminalColors.ENDC}")
...
async def change_presence():
    activities = [
        Activity(type=ActivityType.streaming, name=STATUS1, url=url_status),
        ...
        Activity(type=ActivityType.watching, name=STATUS5)
    ]
...
@bot.slash_command(
    name="saverole_menu",
    description="ตั้งค่าระบบบันทึกบทบาท"
)
async def saverole_setup(interaction: nextcord.Interaction):
    ...
    embed = nextcord.Embed(
        title="🔐 **บันทึกบทบาท**",
        description="``` > เซฟยศ (เซฟไว้ในระบบ)\n > รับยศคืน (ได้ยศคืนตามที่เซฟไว้)```",
        color=COLOR
    )
    embed.set_image(url=Images)
...
class Saverole(nextcord.ui.View):
    ...
    @button(label="เซฟยศ", style=ButtonStyle.blurple, emoji="<a:i7:1210538764905087046>")
    async def save(self, button: nextcord.ui.Button, interaction: Interaction):
        ...
        if user.id in self.cooldowns and datetime.datetime.now() < self.cooldowns[user.id]:
            ...
            embed = Embed(
                title="Cooldown",
                description=f"โปรดรอ {hours} ชั่วโมง {minutes} นาที ก่อนที่จะเซฟยศอีกครั้ง",
                color=COLOR
            )
        ...
        embed = Embed(title="การบันทึกสำเร็จ!", description="บทบาทของคุณถูกบันทึกเรียบร้อยแล้ว 🎉", color=COLOR)
    ...
    @button(label="รับยศคืน", style=ButtonStyle.blurple, emoji="<a:i6:1210538767329267712>")
    async def get(self, button: nextcord.ui.Button, interaction: Interaction):
        ...
        try:
            ...
            await interaction.send(embed=Embed(
                description="**คุณยังไม่ได้บันทึกบทบาทของคุณ.**",
                color=COLOR
            ), ephemeral=True)
            return

        description = "**บทบาทของคุณได้ถูกคืนเรียบร้อยแล้ว**"
        if formatted_roles:
            description += "\n\n**บทบาทที่คืนให้:**\n"
    ...
    @button(label="ดูข้อมูลผู้ใช้", style=ButtonStyle.blurple, emoji="<a:1198631834087592011:1198631834087592011>")
    async def get_user_info(self, button: nextcord.ui.Button, interaction: Interaction):
        ...
        await interaction.response.send_message(content="User info:", embed=user_info_embed, ephemeral=True)


    @nextcord.ui.button(label="คนเซฟยศ", style=nextcord.ButtonStyle.blurple, emoji=emoji_12)
    async def check_saved_roles(self, button: nextcord.Button, interaction: nextcord.Interaction):
        ...
        embed = nextcord.Embed(
            ...
            description=f"จำนวนผู้เซฟยศทั้งหมด: `{saved_roles_count}`",
            color=COLOR
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)

    @nextcord.ui.button(label="ดูประวัติบทบาท", style=nextcord.ButtonStyle.blurple, emoji=emoji_10)
    ...
    @nextcord.ui.button(label="เช็คยศตัวเอง", style=nextcord.ButtonStyle.blurple, emoji=emoji_8)
    async def callback(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
        ...
        if not member_roles:
            embed = nextcord.Embed(
                title="เเจ้งเตือน", 
                description="**คุณไม่มียศ**", 
                color=COLOR
            )
        else:
            embed = nextcord.Embed(
                title="ยศของคุณทั้งหมดมีดังนี้ :", 
                description="\n".join(member_roles),
                color=COLOR
            )
            embed.set_thumbnail(url=interaction.user.display_avatar.url)
            embed.add_field(name="checked !", value=f"**จำนวนยศทั้งหมด {str(num_roles)} ยศ**", inline=False)

            await interaction.send(embed=embed, ephemeral=True)
        
        await interaction.response.send_message(embed=embed, ephemeral=True)
    @button(label="วิธีการใช้งาน", style=ButtonStyle.grey, emoji="<:4983blackcathuh:1198631074058752062>")
    async def usage(self, button: nextcord.ui.Button, interaction: Interaction):
        instructions = "```วิธีการใช้งาน:\n1. คลิกปุ่ม 'เซฟยศ' เพื่อบันทึกยศของคุณ\n2. หากต้องการคืนยศให้กับตัวเอง คลิกปุ่ม 'รับยศคืน'\n3. หากต้องการดูวิธีการใช้งานอื่นๆ คลิกปุ่ม 'วิธีการใช้งาน'\n\nหมายเหตุ : การกดเซฟยศมีคลูดาว 1 วัน\nกรณีดิสโดนยิงหรือคุณออกจากดิสยศคุณจะไม่หาย```"

        embed = Embed(title="วิธีการใช้งานบอท", description=instructions, color=COLOR)
        embed.set_footer(text=f"Made by {name}!")
...
async def attack_setup(interaction: nextcord.Interaction):
    ...
    embed = nextcord.Embed(
        ...
        description='```กดปุ่มด้านล่างเพื่อทำการยิงเว็บ```',
        color=COLOR
    )
    bot_icon_url = str(bot.user.avatar.url) if bot.user.avatar else nextcord.Embed.Empty
    embed.set_image(url=Images)
...
class verify(nextcord.ui.View):
    ...
    async def addrole(self, button: nextcord.Button, interaction: nextcord.Interaction):
        ...
        if not role:
            embed = nextcord.Embed(description="บทบาทที่ถูกตั้งค่าไม่พบในเซิร์ฟเวอร์", color=COLOR)
            await interaction.send(embed=embed, ephemeral=True)
            return

        if role in interaction.user.roles:
            embed = nextcord.Embed(description="คุณอยู่มียศอยู่แล้ว", color=COLOR)
            await interaction.send(embed=embed, ephemeral=True)
            return
        else:
            ...
            embed = nextcord.Embed(description="คุณยืนยันตัวตนสำเร็จ", color=COLOR)
            await interaction.send(embed=embed, ephemeral=True)

            dm = nextcord.Embed(
                title=f"{server_name}",
                description=f"__ข้อมูลการยืนยันตัวตน__\n\nคุณ ได้ยืนยันตัวตนเสร็จสิ้น",
                color=COLOR
                )
            ...
            log = nextcord.Embed(title="รับยศสำเร็จ", description=f"รับยศ `{role}` สำเร็จ", color=COLOR)
...
class webhook(nextcord.ui.View):
    ...
    @nextcord.ui.button(label="เช็คเว็ปฮุค", style=nextcord.ButtonStyle.blurple, emoji="<a:Black_CatLinguinha:1198632831488897107>",custom_id="check")
    ...
    @nextcord.ui.button(label="ลบเว็บฮุค", style=nextcord.ButtonStyle.blurple, emoji="<:dawdas:1202987022994776145>",custom_id="delete")
...
async def verify_setup(interaction: nextcord.Interaction):
    ...
    embed = nextcord.Embed(
        ...
        description="__วิธีการเข้าใช้งานเชิฟเวอร์ดิสคอร์ด__",
        color=COLOR
    )
    embed.set_image(url=Images)
    embed.set_thumbnail(url=server_icon_url)
    embed.add_field(name="Instructions", value="กดปุ่มด่านล่าง ``ยืนยันตัวตน`` เพิ่อเข้าใช้งานดิสคอร์ด", inline=False)
...
class NSFW(View):
    ...
    @button(label="Vtuber", style=ButtonStyle.primary, emoji=emoji_1)
    ...
    @button(label="Pussy", style=ButtonStyle.primary, emoji=emoji_2)
    async def send_anime(self, button: nextcord.Button, interaction: nextcord.Interaction):
        await self.send_nsfw(interaction, "pussy")

    @button(label="Creampie", style=ButtonStyle.primary, emoji=emoji_3)
    ...
    @button(label="Neko", style=ButtonStyle.primary, emoji=emoji_4)
    ...
    @button(label="Random", style=ButtonStyle.primary, emoji=emoji_5)
    ...
    @button(label="Neko Non-NSFW", style=ButtonStyle.primary, emoji=emoji_12)
    ...
    async def send(self, button: nextcord.Button, interaction: nextcord.Interaction):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                await self.handle_error(interaction, 'Sorry, the NSFW API is currently down. Please try again later.')
        except requests.RequestException as e:
            await self.handle_error(interaction, 'Sorry, an error occurred while processing your request. Please try again later.')
    ...
    async def send_nsfw(self, interaction: nextcord.Interaction, category: str):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                await self.handle_error(interaction, 'Sorry, the NSFW API is currently down.')
        except requests.RequestException as e:
            await self.handle_error(interaction, 'Sorry, an error occurred while processing your request. Please try again later.')

    async def handle_error(self, interaction: nextcord.Interaction, error_message: str):
        msg = await interaction.response.send_message(error_message, ephemeral=True)
...
async def nsfw_setup(interaction: nextcord.Interaction):
    ...
    embed.set_footer(text="all features are NSFW")
...
async def setup(interaction: nextcord.Interaction):
    if (interaction.user.id not in OWNER_ID):
        return await interaction.response.send_message(content='[ERROR] No Permission For Use This Command.', ephemeral=True)
    embed = nextcord.Embed()
    embed.title = '───                    FLEXZY STORE [CHNAGE UR OWN]               ───'
    embed.description = f'''
```
─────────────────────────────────────
...
class MyModalFollower(nextcord.ui.Modal):
    def __init__(self):
        super().__init__(title='เข้าสู่ระบบ')
...
class ButtonView_credit(nextcord.ui.View):
    ...
    async def send_message(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
        ...
        if target_channel:
            embed = Embed(title="Thank you na ❤️", description=f"ขอบคุณ {interaction.user.mention} สสสส", color=nextcord.Color.green())
...
async def credit(ctx: commands.Context):
    ...
    if user.guild_permissions.administrator:
        ...
        embed = Embed(title="**__Miyako Server__**", description="ให้เครดิตโดยกดปุ่มด้านล่างนะ.")
...
class Donate(nextcord.ui.Modal):
    ...
    async def callback(self, interaction: nextcord.Interaction):
        ...
        if response.status_code == 200 and redeemdata["status"]["code"] == "SUCCESS" :
            ...
            done = nextcord.Embed(description=f'`✅ โดเนทสำเร็จ` `|` จำนวน `{amount}` บาท',color=COLOR)
...
@bot.slash_command(guild_ids=[Guild_ID],description="Donate na kubb ❤️")
async def donate(interaction: nextcord.Interaction):
    embedhypre = nextcord.Embed(title="Server **__Miyako V1__**",description=f"**กดปุ่มด้านล่างเพื่อโดเนท**",color=COLOR)
...
async def on_voice_state_update(member, before, after):
    ...
    if before.channel != after.channel or before.self_mute != after.self_mute or before.self_deaf != after.self_deaf:
        ...
        if before.self_mute != after.self_mute:
            if after.self_mute:
                embed.description += ' 🔇 (Server muted)'
            else:
                embed.description += ' 🔊 (Server unmuted)'

        if before.self_deaf != after.self_deaf:
            if after.self_deaf:
                embed.description += ' 🔇 (Server deafened)'
            else:
                embed.description += ' 🔊 (Server undeafened)'
...
@bot.slash_command(
    name="checkip",
    description="🔍 Check details for a given IP address"
)
...
@bot.slash_command(
    name="dm",
    description="✉️ Send a private message to a user."
)
async def dm(ctx, recipient: nextcord.User, msg: str):
    ...
    if author:
        ...
        if member and member.guild_permissions.administrator:
            ...
            await ctx.send(f"✉️ Sent message to {recipient.mention} successfully!", ephemeral=True)

            log_channel = bot.get_channel(all_log_channel)
            if log_channel:
                embed_log = nextcord.Embed(
                    title="Direct Message Sent",
                    description=f"✉️ Direct message sent to {recipient.mention} by {author.mention}.",
                    color=COLOR
                )
                await log_channel.send(embed=embed_log)
        else:
        ...
        await ctx.send("❌ Error: Unable to retrieve author information.", ephemeral=True)

def generate_embed(author, msg, image_url=None):
    embed = nextcord.Embed(title="Message from Admin", description=msg, color=COLOR)
    if image_url:
...
async def botinfo(ctx: commands.Context):
    ...
    embed.add_field(name='Owner 👑', value=(f"{name}"), inline=False)
...
async def ping(ctx):
    ...
    if latency < 100:
        ...
        status = '🟡 ``Online, but slightly slow``'
    else:
        status = '🔴 ``Online, but experiencing high latency``'
...
@bot.slash_command(
    name="create_invite",
    description="📨 Create a permanent invite link for the server."
)
async def createserver(ctx):
    guild = ctx.guild
    if guild is None:
        await ctx.send("❌ This command can only be used in a server context.")
        return
    
    user = ctx.user
    print(f"🔗 User: {user} creates an invite.")
    try:
    ...
    if member and member.guild_permissions.administrator:
        ...
        await ctx.send(f"📨 Here is the permanent invite link for the server: {server_invite.url}", ephemeral=True)

        log_channel = bot.get_channel(all_log_channel)
        if log_channel:
            embed = nextcord.Embed(
                title="Server Invite Created",
                description=f"📨 Permanent invite link for the server created by {user.mention}.",
                color=COLOR
            )
...
@bot.slash_command(
    name="checkvoice",
    description="🔊 Check the status of users in each voice channel category."
)
            ...
            for category, channels in categories:
                ...
                for channel in channels:
                    if isinstance(channel, nextcord.VoiceChannel):
                        ...
                        channels_info += f"{channel.name} - {len(channel.voice_states)} 👤\n"
...
@bot.slash_command(
    name="clear",
    description="🗑️ Clear all messages in the channel and recreate the channel in the same category."
)
async def clear(interaction: nextcord.Interaction):
    ...
    if any(admin_role in user.roles for admin_role in interaction.guild.roles):
        try:
            ...
            await new_channel.send("All messages have been cleared in this channel. New channel created.")

            await interaction.response.send_message("🗑️ Channel cleared and recreated in the same category and position. "
...
async def support(ctx: commands.Context, message: str):
    ...
    if not message:
        await ctx.send("❌ Please send a message with your support request.")
        return
    ...
    if admin_channel:
        embed = nextcord.Embed(
            ...
            description=f"🆘 Support request from {user.mention}:\n\n```{message}```",
            color=COLOR,
        ...
        await ctx.response.send_message(content="✉️ Support request sent.", ephemeral=True)

        await admin_channel.send(embed=embed)

    else:
        await ctx.send("❌ Admin support channel not found. Please contact an admin for assistance.")

# userinfo
@bot.slash_command(
    name="userinfo",
    description="แสดงข้อมูลของสมาชิก"
)
...
@bot.slash_command(
    name="invite_check",
    description="📊 Show the top 3 members who invited the most in the server."
)
async def invite_command(ctx):
    if ctx.user.guild_permissions.administrator:
        ...
        if log_channel:
            embed_log = nextcord.Embed(
                title="Invite Check",
                description=f"📊 Invite check command executed by {ctx.user.mention}.",
                color=COLOR
            )
            await log_channel.send(embed=embed_log)
    else:
        await ctx.response.send_message("⛔ [ERROR] No Permission For Use This Command.", ephemeral=True)
        ...
        if '.onrender.com' in link or '.csb.app' in link:
            ...
            if response['stat'] == 'ok':
                emlog = nextcord.Embed(
                    title='Start.',
                    description=f'\n > ผู้ใช้บริการ : {interaction.user.mention}\n> สถานะ : **🟢**\n> link : {link}',
                    color=COLOR
                )
                await log.send(embed=emlog)
                embed = nextcord.Embed(
                    title='✅ เพิ่มลงในระบบสำเร็จ',
                    description=f'เราได้เพิ่ม[ลิงก์ของคุณ]({link})ไว้ในระบบเรียบร้อยแล้ว!',
                    color=COLOR
                )
                await interaction.send(embed=embed, ephemeral=True)
            else:
                if 'already_exists' in response['error']['type']:
                    embed = nextcord.Embed(
                        ...
                        description=f'เนื่องจากลิงก์ของคุณนั้นได้อยู่ในระบบเรียบร้อยแล้ว!',
                        color=COLOR
                    )
                    await interaction.send(embed=embed, ephemeral=True)
                else:
                    embed = nextcord.Embed(
                        title='❌ ทำรายการไม่สำเร็จ',
                        description=f'เนื่องจากลิงก์ที่คุณระบุมาไม่ถูกต้อง กรุณาตรวจสอบใหม่อีกครั้ง!',
                        color=COLOR
                    )
                    await interaction.send(embed=embed, ephemeral=True)
        else:
            embed = nextcord.Embed(
                title='❌ ทำรายการไม่สำเร็จ',
                description=f'กรุณาใส่ลิงก์ของ Render หรือ csb.app เท่านั้นนะครับ!',
                color=COLOR
            )
            await interaction.send(embed=embed, ephemeral=True)
    else:
        await interaction.send('# กรุณาใช้คำสั่งในช่องให้ถูกต้องนะ', ephemeral=True)
    ...
    if author.guild_permissions.ban_members:
        try:
            ...
            if log_channel:
                embed = nextcord.Embed(
                    title="Member Banned",
                    description=f"⛔ {member.mention} has been banned by {author.mention}.\n\nReason: {reason}",
                    color=nextcord.Color.red()
                )
                await log_channel.send(embed=embed)
        except nextcord.Forbidden:
        ...
        try:
            ...
            if log_channel:
                embed = nextcord.Embed(
                    ...
                    description=f"🔓 {user.mention} has been unbanned by {author.mention}.\n\nReason: {reason}",
                    color=nextcord.Color.green()
                )
                await log_channel.send(embed=embed)
        except nextcord.NotFound:
...
class AddRoleView(nextcord.ui.View):
    ...
    async def add_role(self, interaction: nextcord.Interaction, role_id: int):
        try:
            ...
            if role:
                if role in interaction.user.roles:
                    await interaction.response.send_message("```คุณมีบทบาทนี้อยู่แล้ว 💢!```", ephemeral=True)
                elif role.position >= interaction.guild.me.top_role.position:
                    await interaction.response.send_message("```ยศสูงเกินไปเราให้ไม่ได้ 💢!```", ephemeral=True)
                else:
                    await interaction.user.add_roles(role)
                    await interaction.response.send_message(f"เพิ่มบทบาท ``{role.name}`` สำเร็จแล้ว!", ephemeral=True)

                    log_channel = bot.get_channel(all_log_channel)
                    if log_channel:
                        embed = nextcord.Embed(
                            title="เพิ่มบทบาท",
                            description=f"{interaction.user.mention} เพิ่มบทบาทให้ **{role.name}** เรียบร้อย.\n",
                            color=COLOR,
                        )
                ...
                await interaction.response.send_message("```ไม่พบบทบาท 💢!```", ephemeral=True)
        except Exception as e:
            await self.handle_error(interaction, f"An unexpected error occurred: {e}")
    ...
    @nextcord.ui.button(label=button_role_6, style=nextcord.ButtonStyle.primary, emoji=emoji_14)
    async def role6_button(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
        await self.add_role(interaction, role_6)
        
    @nextcord.ui.button(label=button_role_7, style=nextcord.ButtonStyle.primary, emoji=emoji_15)
    async def role7_button(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
        await self.add_role(interaction, role_7)
...
async def global_error_handler(interaction: nextcord.Interaction, error_message: str):
    msg = await interaction.response.send_message(error_message, ephemeral=True)
    await asyncio.sleep(5)
    await msg.delete()
...
async def add_role_menu(interaction: nextcord.Interaction):
    ...
    embed = nextcord.Embed(
        title="รับยศเพื่อเห็นห้องฟรี",
        description=f"```กดปุ่มด้านล่างเพื่อทำการรับยศ```\n Please read <#1192105533025566790>.",
        color=COLOR
    )
    embed.set_image(url=Images)
    embed.set_footer(text=f"Made By {name}") 
    await interaction.channel.send(embed=embed, view=AddRoleView())
    await interaction.response.send_message(content='[SUCCESS] Done.', ephemeral=True)
...
async def invitebot(ctx: nextcord.Interaction):
    ...
    embed = nextcord.Embed(
        title="Invite Link for the Bot ❤️",
...
class SPAM(nextcord.ui.Modal):
  def __init__(self):
    super().__init__(title='ยิงเบอร์ฟรี')
...
class Share_gaverole(View):
    ...
    @nextcord.ui.button(label="อนุมัติ", style=nextcord.ButtonStyle.green, emoji=emoji_12)
    async def approve(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
        ...
        if role:  
            ...
            embed = nextcord.Embed(
                title="✅ ได้รับยศแล้ว",
                description=f"`ผู้ดูแลระบบได้อนุมัติให้ยศเรียบร้อยแล้ว!`",
                color=COLOR
            )
            await channel.send(embed=embed)
        else:
            print("บทบาทไม่พร้อมใช้งานหรือไม่พบบทบาทนี้ในเซิร์ฟเวอร์")


    @nextcord.ui.button(label="ไม่อนุมัติ", style=nextcord.ButtonStyle.red, emoji=emoji_8)
    async def decline(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
        ...
        embed = nextcord.Embed(
            title="❌ ไม่ได้รับยศ",
            description=f"`ผู้ดูแลระบบไม่ยินยอมอนุมัติให้ยศกรุณาตรวจสอบข้อมูลแล้วลองใหม่อีกครั้ง!` 💢",
            color=COLOR
        )
        await channel.send(embed=embed)
...
async def on_message(message):
    if str(message.channel.id) == str(channel_users):
        ...
        embed1 = nextcord.Embed(
            title="อยู่ระหว่างการตรวจสอบโดยผู้ดูแลระบบ",
            description=f"```เราได้ส่งข้อมูลของคุณไปยังผู้ดูแลระบบเพื่อตรวจสอบ โปรดรอการตอบกลับ```",
            color=COLOR
        )
        embed = nextcord.Embed(
            title="ได้รับการส่งหลักฐานแล้ว",
            description=f"ส่งโดย : {message.author.mention}\nโปรดตรวจสอบหลักฐานที่ : <#{channel_users}>",
            color=COLOR
        )
...
async def copyright(ctx: commands.Context):
    ...
    await ctx.send(copyright_info, ephemeral=True)
...
class Topup(Modal):
	def __init__(self):
		super().__init__(title="เติมเงินอัตโนมัติตลอด 24 ชั่วโมง")
		self.link_angpao = nextcord.ui.TextInput(label="กรอกลิงก์ซองของขวัญ", required=True)
		self.add_item(self.link_angpao)
	
	async def callback(self, interaction: Interaction):
		msg = await interaction.response.send_message("checking..", ephemeral=True)
		if not "https://gift.truemoney.com/campaign/?v=" in self.link_angpao.value:
			embed = nextcord.Embed(title="❌ เติมเงินไม่สำเร็จ",description="ลิงก์ซองของขวัญไม่ถูกต้อง กรุณาตรวจสอบใหม่อีกครั้ง",color=COLOR)
			await msg.edit(content=None, embed=embed)
		else:
			response = requests.post(f"https://restapi.kdkddmdmdd.repl.co/undefined_store/topupwallet",json={"mobile": phoneNumber,"link": str(self.link_angpao.value)}).json()
			if response["status"] == True:
				amount = float(response['amount'])
				try:
					with open(f"./data/users/{interaction.user.name}.json", "r") as fileUser:
						data = json.load(fileUser)[f'{interaction.user.name}']
						with open(f"./data/users/{interaction.user.name}.json", "w+") as fileUsers:
							updateMoney = float(data['amount']) + float(amount)
							updateAccumulate = float(data['accumulate']) + float(amount)
							updateData = save(interaction.user.name, interaction.id, float(updateMoney), float(updateAccumulate))
							fileUsers.write(updateData)
					with open(f"./data/users/{interaction.user.name}.json", "r") as fileUser:
						data = json.load(fileUser)[f'{interaction.user.name}']
						embed = nextcord.Embed(title="✔️ เติมเงินสำเร็จแล้ว",description=f"คุณได้เติมเงินมาจำนวน +{amount}0 บาทสำเร็จเรียบร้อยแล้ว !",color=COLOR)
						embed.set_footer(text=f"ยอดเงินคงเหลือ : {data['amount']}0 บาท")
						await msg.edit(content=None, embed=embed) 
				except FileNotFoundError:
					with open(f"./data/users/{interaction.user.name}.json", "w+") as fileUsers:
						updateData = save(interaction.user.name, interaction.id, float(amount), float(amount))
						fileUsers.write(updateData)
					with open(f"./data/users/{interaction.user.name}.json", "r") as fileUser:
						data = json.load(fileUser)[f'{interaction.user.name}']
						embed = nextcord.Embed(title="✔️ เติมเงินสำเร็จแล้ว",description=f"คุณได้เติมเงินมาจำนวน +{amount}0 บาทสำเร็จเรียบร้อยแล้ว !",color=COLOR)
				...
				embed = nextcord.Embed(title="❌ เติมเงินไม่สำเร็จ",description="ลิงก์ซองของขวัญไม่ถูกต้อง / รับเงินไปแล้ว",color=COLOR)
				await msg.edit(content=None, embed=embed)
...
class SendDm1(View):
	...
	async def callback(self, button: nextcord.Button, interaction: Interaction):
		user = await bot.fetch_user(int(self.id))
		button.label = "ยืนยันแล้ว !"
		button.style = nextcord.ButtonStyle.grey
		button.disabled = True
		embed = nextcord.Embed(title=f"{interaction.guild.name} | ประวัติการสั่งซื้อสินค้า",description="## ข้อมูลการสั่งสินค้า\n\n  🛒ชื่อสินค้า : Nitro Classic (1เดือน)\n  ⭕ สถานะ : **ทำรายการสำเร็จ**",color=COLOR)
		embed.set_image(url="https://cliply.co/wp-content/uploads/2021/08/472108170_THANK_YOU_STICKER_400px.gif")
...
class SendDm3(View):
	...
	async def callback(self, button: nextcord.Button, interaction: Interaction):
		...
		button.style = nextcord.ButtonStyle.grey
		button.disabled = True
		embed = nextcord.Embed(title=f"{interaction.guild.name} | ประวัติการสั่งซื้อสินค้า",description="## ข้อมูลการสั่งสินค้า\n\n  🛒ชื่อสินค้า : Nitro Booster (1เดือน)\n  ⭕ สถานะ : **ทำรายการสำเร็จ**",color=COLOR)
		embed.set_image(url="https://cliply.co/wp-content/uploads/2021/08/472108170_THANK_YOU_STICKER_400px.gif")
		await user.send(embed=embed)
		await interaction.response.edit_message(view=self)
...
class SendDm4(View):
	...
	async def callback(self, button: nextcord.Button, interaction: Interaction):
		...
		button.style = nextcord.ButtonStyle.grey
		button.disabled = True
		embed = nextcord.Embed(title=f"{interaction.guild.name} | ประวัติการสั่งซื้อสินค้า",description="## ข้อมูลการสั่งสินค้า\n\n  🛒ชื่อสินค้า : Nitro Booster (1ปี)\n  ⭕ สถานะ : **ทำรายการสำเร็จ**",color=COLOR)
		embed.set_image(url="https://cliply.co/wp-content/uploads/2021/08/472108170_THANK_YOU_STICKER_400px.gif")
		await user.send(embed=embed)
		await interaction.response.edit_message(view=self)

class NitroClassic(Modal):
	def __init__(self):
		super().__init__(title="สั่งซื้อ Nitro Classic")
		self.username = nextcord.ui.TextInput(label="USERNAME", placeholder="Email or Phone", required=True)
		self.password = nextcord.ui.TextInput(label="PASSWORD", placeholder="xxxxxxxxxxxxx", required=True)
		self.add_item(self.username)
		self.add_item(self.password)
	
	async def callback(self, interaction: Interaction):
		user = interaction.user
		log = nextcord.utils.get(interaction.guild.channels, id=int(id))
		with open(f"./data/users/{interaction.user.name}.json", "r") as user:
			data = json.load(user)[f'{interaction.user.name}']
			broken = float(data['amount']) - float(49.0)
			with open(f"./data/users/{interaction.user.name}.json", "w+") as userupdate:
				new = save(interaction.user.name, interaction.user.id, float(broken), float(data['accumulate']))
				userupdate.write(new)
			
			embed = nextcord.Embed(title="สั่งซื้อสินค้าสำเร็จแล้ว",description=f"กรุณารอแอดมินทำรายการสักครู่ไม่เกิน 10 นาทีหลังจากทำรายการสั่งซื้อ",color=COLOR)
			embed.set_footer(text=f"ยอดเงินคงเหลือ : {broken}0 บาท")
			await interaction.response.send_message(embed=embed, ephemeral=True)
			
			us = await bot.fetch_user(int(interaction.user.id))
			embed50= nextcord.Embed(title=f"{interaction.guild.name} | ประวัติการสั่งซื้อสินค้า",description="## ข้อมูลการสั่งสินค้า\n\n  🛒ชื่อสินค้า : Nitro Classic (1เดือน)\n  ⭕ สถานะ : **กำลังดำเนินรายการ...**",color=COLOR)
			await us.send(embed=embed50)
		
		
		embed2 = nextcord.Embed(title=f"ประวัติการสั่งซื้อ Nitro Clsssic",description=f"## > รายละเอียดการสั่งซื้อ :\n\n  👤 ผู้ที่สั่งซื้อ : {interaction.user.mention}\n  📖 ประเภทการสั่งซื้อ : Nitro Classic (1เดือน)\n  ☁ Username : {self.username.value}\n  ☁ Password : {self.password.value}**\n\nเมื่อคุณทำรายการสำเร็จแล้วกรุณากดปุ่มยืนยันสินค้านี้เพื่อรายงานไปยังลูกค้า**",color=COLOR)
		await log.send(embed=embed2, view=SendDm1(interaction.user.id))

class NitroClassic2(Modal):
	def __init__(self):
		super().__init__(title="สั่งซื้อ Nitro Classic")
		self.username = nextcord.ui.TextInput(label="USERNAME", placeholder="Email or Phone", required=True)
		self.password = nextcord.ui.TextInput(label="PASSWORD", placeholder="xxxxxxxxxxxxx", required=True)
	...
	async def callback(self, interaction: Interaction):
		...
		with open(f"./data/users/{interaction.user.name}.json", "r") as user:
			data = json.load(user)[f'{interaction.user.name}']
			broken = float(data['amount']) - float(559.0)
			with open(f"./data/users/{interaction.user.name}.json", "w+") as userupdate:
				new = save(interaction.user.name, interaction.user.id, float(broken), float(data['accumulate']))
				userupdate.write(new)
		...
		embed2 = nextcord.Embed(title=f"ประวัติการสั่งซื้อ Nitro Clsssic",description=f"## > รายละเอียดการสั่งซื้อ :\n\n  👤 ผู้ที่สั่งซื้อ : {interaction.user.mention}\n  📖 ประเภทการสั่งซื้อ : Nitro Classic (1ปี)\n  ☁ Username : {self.username.value}\n  ☁ Password : {self.password.value}**\n\nเมื่อคุณทำรายการสำเร็จแล้วกรุณากดปุ่มยืนยันสินค้านี้เพื่อรายงานไปยังลูกค้า**",color=COLOR)
		await log.send(embed=embed2, view=SendDm2(interaction.user.id))
		
class NitroBoot(Modal):
	def __init__(self):
		super().__init__(title="สั่งซื้อ Nitro Booster")
		self.username = nextcord.ui.TextInput(label="USERNAME", placeholder="Email or Phone", required=True)
		self.password = nextcord.ui.TextInput(label="PASSWORD", placeholder="xxxxxxxxxxxxx", required=True)
		self.add_item(self.username)
		self.add_item(self.password)
	...
	async def callback(self, interaction: Interaction):
		...
		with open(f"./data/users/{interaction.user.name}.json", "r") as user:
			data = json.load(user)[f'{interaction.user.name}']
			broken = float(data['amount']) - float(110.0)
			with open(f"./data/users/{interaction.user.name}.json", "w+") as userupdate:
				new = save(interaction.user.name, interaction.user.id, float(broken), float(data['accumulate']))
				userupdate.write(new)
		...
		embed2 = nextcord.Embed(title=f"ประวัติการสั่งซื้อ Nitro Clsssic",description=f"## > รายละเอียดการสั่งซื้อ :\n\n  👤 ผู้ที่สั่งซื้อ : {interaction.user.mention}\n  📖 ประเภทการสั่งซื้อ : Nitro Booster (1เดือน)\n  ☁ Username : {self.username.value}\n  ☁ Password : {self.password.value}**\n\nเมื่อคุณทำรายการสำเร็จแล้วกรุณากดปุ่มยืนยันสินค้านี้เพื่อรายงานไปยังลูกค้า**",color=COLOR)
		await log.send(embed=embed2, view=SendDm3(interaction.user.id))

class NitroBoot2(Modal):
	def __init__(self):
		super().__init__(title="สั่งซื้อ Nitro Booster")
		self.username = nextcord.ui.TextInput(label="USERNAME", placeholder="Email or Phone", required=True)
		self.password = nextcord.ui.TextInput(label="PASSWORD", placeholder="xxxxxxxxxxxxx", required=True)
	...
	async def callback(self, interaction: Interaction):
		...
		with open(f"./data/users/{interaction.user.name}.json", "r") as user:
			data = json.load(user)[f'{interaction.user.name}']
			broken = float(data['amount']) - float(1100.0)
			with open(f"./data/users/{interaction.user.name}.json", "w+") as userupdate:
				new = save(interaction.user.name, interaction.user.id, float(broken), float(data['accumulate']))
				userupdate.write(new)
		...
		embed2 = nextcord.Embed(title=f"ประวัติการสั่งซื้อ Nitro Clsssic",description=f"## > รายละเอียดการสั่งซื้อ :\n\n  👤 ผู้ที่สั่งซื้อ : {interaction.user.mention}\n  📖 ประเภทการสั่งซื้อ : Nitro Booster (1ปี)\n  ☁ Username : {self.username.value}\n  ☁ Password : {self.password.value}**\n\nเมื่อคุณทำรายการสำเร็จแล้วกรุณากดปุ่มยืนยันสินค้านี้เพื่อรายงานไปยังลูกค้า**",color=COLOR)
		await log.send(embed=embed2, view=SendDm4(interaction.user.id))
			
class DropDown(Select):
	def __init__(self):
		option = [
			nextcord.SelectOption(label="Nitro Classic / 1 เดือน",description="ราคา 49.00 บาท",emoji="<a:paymentnitro:1175846377926959247>", value="1"),
			nextcord.SelectOption(label="Nitro Classic / 1 ปี",description="ราคา 559.00 บาท",emoji="<a:paymentnitro:1175846377926959247>", value="2")
		]
		super().__init__(placeholder="เลือกแผนการของคุณ", options=option)
	...
	async def callback(self, interaction: Interaction):
		with open(f"./data/users/{interaction.user.name}.json", "r") as fileUser:
			...
			if self.values[0] == "1":
				if float(data['amount']) < float(49.0):
					broken = float(49.0) - float(data['amount'])
					embed = nextcord.Embed(title="❌ ยอดเงินคงเหลือไม่เพียงพอ",description=f"กรุณาเติมเงินเพิ่มก่อนสั่งซื้อรายการนี้ (ขาดเงินอีก {float(broken)}0 บาท)",color=COLOR)
					embed.set_footer(text=f"ยอดเงินคงเหลือ : {data['amount']}0 บาท")
					await interaction.response.edit_message(embed=embed, view=None)
				else:
					await interaction.response.send_modal(NitroClassic())
			if self.values[0] == "2":
				if float(data['amount']) < float(559.0):
					broken = float(559.0) - float(data['amount'])
					embed = nextcord.Embed(title="❌ ยอดเงินคงเหลือไม่เพียงพอ",description=f"กรุณาเติมเงินเพิ่มก่อนสั่งซื้อรายการนี้ (ขาดเงินอีก {float(broken)}0 บาท)",color=COLOR)
					embed.set_footer(text=f"ยอดเงินคงเหลือ : {data['amount']}0 บาท")
					await interaction.response.edit_message(embed=embed, view=None)
				else:
					await interaction.response.send_modal(NitroClassic2())
					
class DropDown2(Select):
	def __init__(self):
		option = [
			nextcord.SelectOption(label="Nitro Booster / 1 เดือน",description="ราคา 110.00 บาท",emoji="<a:paymentnitro:1175846377926959247>", value="1"),
			nextcord.SelectOption(label="Nitro Booster / 1 ปี",description="ราคา 1,100.00 บาท",emoji="<a:paymentnitro:1175846377926959247>", value="2")
		]
		super().__init__(placeholder="เลือกแผนการของคุณ", options=option)
	...
	async def callback(self, interaction: Interaction):
		with open(f"./data/users/{interaction.user.name}.json", "r") as fileUser:
			...
			if self.values[0] == "1":
				if float(data['amount']) < float(110.0):
					broken = float(110.0) - float(data['amount'])
					embed = nextcord.Embed(title="❌ ยอดเงินคงเหลือไม่เพียงพอ",description=f"กรุณาเติมเงินเพิ่มก่อนสั่งซื้อรายการนี้ (ขาดเงินอีก {float(broken)}0 บาท)",color=COLOR)
					embed.set_footer(text=f"ยอดเงินคงเหลือ : {data['amount']}0 บาท")
					await interaction.response.edit_message(embed=embed, view=None)
				else:
					await interaction.response.send_modal(NitroBoot())
			if self.values[0] == "2":
				if float(data['amount']) < float(1100.0):
					broken = float(1100.0) - float(data['amount'])
					embed = nextcord.Embed(title="❌ ยอดเงินคงเหลือไม่เพียงพอ",description=f"กรุณาเติมเงินเพิ่มก่อนสั่งซื้อรายการนี้ (ขาดเงินอีก {float(broken)}0 บาท)",color=COLOR)
					embed.set_footer(text=f"ยอดเงินคงเหลือ : {data['amount']}0 บาท")
					await interaction.response.edit_message(embed=embed, view=None)
				else:
					await interaction.response.send_modal(NitroBoot2())
...
class Component2(View):
	def __init__(self):
		super().__init__(timeout=None)
		self.add_item(DropDown2())
	
class nitro_ehh(View):
	def __init__(self):
		super().__init__(timeout=None)
		self.btn = nextcord.ui.Button(label="วิธีการใช้งาน", style=ButtonStyle.link, url=url)
		self.add_item(self.btn)
		self.btn.disabled = True
		
	@nextcord.ui.button(label="เติมเงิน", style=ButtonStyle.red, emoji="🧧")
	async def callback1(self, button: nextcord.Button, ctx: Interaction):
		await ctx.response.send_modal(Topup())
		
	@nextcord.ui.button(label="สิทธิ Nitro ยอดนิยม", style=ButtonStyle.primary, emoji="📖")
	async def callback2(self, button: nextcord.Button, ctx: Interaction):
		await ctx.response.send_message("https://i.ibb.co/fCxPC8H/Screenshot-2024-04-30-043836.png", ephemeral=True)
	
	@nextcord.ui.button(label="Nitro Boot", style=ButtonStyle.green)
	async def callback3(self, button: nextcord.Button, ctx: Interaction):
		try:
			with open(f"./data/users/{ctx.user.name}.json", "r") as fileUser:
				data = json.load(fileUser)[f'{ctx.user.name}']
				embed = nextcord.Embed(title="Nitro Booster",color=0xFCE5CD)
				embed.set_image(url="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRBxE9r0BoFbCUcy0_mqTSVNoPsDAolugWVsw&usqp=CAU")
				await ctx.response.send_message(embed=embed, view=Component2(), ephemeral=True)
		except FileNotFoundError:
			embed = nextcord.Embed(title="❌ ไม่พบข้อมูลบัญชีของคุณ",description="ในระบบไม่มีบัญชีของคุณอยู่กรุณาเติมเงินเพื่อเข้าระบบ",color=COLOR)
			await ctx.response.send_message(embed=embed, ephemeral=True)
	
	@nextcord.ui.button(label="Nitro Classic", style=ButtonStyle.green)
	async def callback4(self, button: nextcord.Button, ctx: Interaction):
		try:
			with open(f"./data/users/{ctx.user.name}.json", "r") as fileUser:
				data = json.load(fileUser)[f'{ctx.user.name}']
				embed = nextcord.Embed(title="Nitro Classic",color=0xFCE5CD)
				embed.set_image(url="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRBxE9r0BoFbCUcy0_mqTSVNoPsDAolugWVsw&usqp=CAU")
				await ctx.response.send_message(embed=embed, view=Component(), ephemeral=True)
		except FileNotFoundError:
			embed = nextcord.Embed(title="❌ ไม่พบข้อมูลบัญชีของคุณ",description="ในระบบไม่มีบัญชีของคุณอยู่กรุณาเติมเงินเพื่อเข้าระบบ",color=COLOR)
			await ctx.response.send_message(embed=embed, ephemeral=True)
		
	@nextcord.ui.button(label="เช็คยอดเงินคงเหลือ", style=ButtonStyle.primary, emoji="💰")
	async def callback5(self, button: nextcord.Button, ctx: Interaction):
		try:
			with open(f"./data/users/{ctx.user.name}.json", "r") as fileUser:
				data = json.load(fileUser)[f'{ctx.user.name}']
				embed = nextcord.Embed(title="👤 ข้อมูลบัญชีส่วนตัวของคุณ",description=f"ยอดเงินคงเหลือ : {data['amount']}0 บาท\nยอดเติมเงินสะสมรวม : {data['accumulate']}0 บาท",color=COLOR)
				await ctx.response.send_message(embed=embed, ephemeral=True)
		except FileNotFoundError:
			embed = nextcord.Embed(title="❌ ไม่พบข้อมูลบัญชีของคุณ",description="ในระบบไม่มีบัญชีของคุณอยู่กรุณาเติมเงินเพื่อเข้าระบบ",color=COLOR)
			await ctx.response.send_message(embed=embed, ephemeral=True)
...
@bot.slash_command(
    name="nitro_menu",
    description="Send nitro menu."
)
async def nitro(interaction: Interaction):
    if interaction.user.name == name:
        embed = nextcord.Embed(title="``Bot server Nitro``",
                               description="\n```[+] Nitro Boost 1m 110\n[+] Nitro Boost 1Y 1100```\n\n```[+] Nitro Classic 1m 49\n[+] Nitro Classic 1Y 559```",
                               color=COLOR)
        embed.set_image(url="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRP5fD817sDYITB81E5zuPKgtOtrW-xAbh5Ow&usqp=CAU")
        await interaction.response.send_message(embed=embed, view=nitro_ehh())
